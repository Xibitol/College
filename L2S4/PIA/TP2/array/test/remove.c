...
int test_remove(void){
	...
	{
		a = array_remove_value(a, pos);

		assert(array_get_length(a) == sizeof vs/sizeof *vs);
		assert(a[pos] != v);
		for(unsigned int i = 0; i < array_get_length(a); i++)
			assert(a[i] == i);

		a = array_remove_values(a, 0, sizeof vs/sizeof *vs - 1);

		assert(array_get_length(a) == 1);
		assert(a[0] == vs[sizeof vs/sizeof *vs - 1]);
	}
