...
static void* clientWorker(void* arg){
	...
	int takenSeats = 0;
	if(sem_getvalue(&waitingSeats, &takenSeats) == -1){
		perror(EXEC_NAME), exitCode = EXIT_FAILURE;
	...
	if(takenSeats > MAX_WAITING_SEATS){
		printf("Patient n°%lu: Plus de place, je vais mourir chez moi.\n", tid);

		pthread_detach(tid);
		return arg;
	}

	printf("Patient n°%lu: Il reste %d place(s) libre(s).\n",
		tid, MAX_WAITING_SEATS - takenSeats
	);
