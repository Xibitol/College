...
bool fraction_init(void){
	...
	strReprs = malloc(__SIZEOF_POINTER__);
...
bool fraction_finish(void){
	...
	while(strReprCount > 0) free(strReprs[--strReprCount]);
	free(strReprs);
...
const char *fraction_to_string(const Fraction *fraction){
	...
	if(pos > 0) strReprs = realloc(strReprs, __SIZEOF_POINTER__*strReprCount);
	strReprs[pos] = malloc(FRACTION_FORMAT_MAX_LENGTH);
